...
namespace yabloc
{
GraphSegment::GraphSegment(rclcpp::Node * node)
: target_height_ratio_(node->declare_parameter<float>("target_height_ratio")),
  target_candidate_box_width_(node->declare_parameter<int>("target_candidate_box_width"))
{
  using std::placeholders::_1;

  const double sigma = node->declare_parameter<double>("sigma");
  const float k = node->declare_parameter<float>("k");
  const int min_size = node->declare_parameter<double>("min_size");
  segmentation_ = cv::ximgproc::segmentation::createGraphSegmentation(sigma, k, min_size);

  // additional area pickup module
  if (node->declare_parameter<bool>("pickup_additional_areas", true)) {
    similar_area_searcher_ = std::make_unique<SimilarAreaSearcher>(
      node->declare_parameter<float>("similarity_score_threshold"));
  }
}
...
std::pair<cv::Mat, cv::Mat> GraphSegment::execute(const cv::Mat & image)
{
  cv::Mat resized;
  ...
  return {output_image, draw_image(image, debug_image)};
}

cv::Mat GraphSegment::draw_image(const cv::Mat & raw_image, const cv::Mat & debug_image)
{
  cv::Mat show_image;
